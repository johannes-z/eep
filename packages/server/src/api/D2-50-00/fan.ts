...
const allD2FunctionIds = Object.keys(getProtocolFunctions('D2-50-00'));

function speedFunctions(supportedFunctions?: readonly string[]) {
  return getSupportedProtocolFunctions('D2-50-00', supportedFunctions ?? allD2FunctionIds).filter(
    (item) => item.kind === 'speed',
  );
}

function offFunction(supportedFunctions?: readonly string[]) {
  return getSupportedProtocolFunctions('D2-50-00', supportedFunctions ?? allD2FunctionIds).find(
...
export function fanSupportedPresets(supportedFunctions?: readonly string[]): D2FanPreset[] {
  return getSupportedProtocolFunctions('D2-50-00', supportedFunctions ?? allD2FunctionIds)
