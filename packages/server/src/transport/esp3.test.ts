...
import { buildUteTeachInResponse, Esp3Parser, parseRadioERP1, type Esp3Frame } from './esp3';
...
test('builds a successful UTE teach-in response', () => {
  const response = buildUteTeachInResponse(0xffe76681, 0x05010203, [0, 0, 0, 0, 0xd2, 0x50, 0]);
  const parser = new Esp3Parser();
  const parsed = parser.push(response);

  expect(parsed).toHaveLength(1);
  expect(parsed[0].packetType).toBe(1);
  expect(parsed[0].data.slice(0, 8)).toEqual([0xd4, 0x91, 0, 0, 0, 0xd2, 0x50, 0]);
  expect(parsed[0].optionalData.slice(0, 5)).toEqual([3, 0x05, 0x01, 0x02, 0x03]);
});

test('parses a UTE query in wire order', () => {
  const payload = [0x80, 0xff, 0x0b, 0, 0xd2, 0x50, 0];
...
test('encodes every UTE response result', () => {
  const requestPayload = [0x80, 0xff, 0x0b, 0, 0xd2, 0x50, 0];
  const responses = [
    ['general', 0x81],
  ...
  for (const [response, control] of responses) {
    ...
    expect(parsed[0].data.slice(0, 8)).toEqual([0xd4, control, 0xff, 0x0b, 0, 0xd2, 0x50, 0]);
  }
});
